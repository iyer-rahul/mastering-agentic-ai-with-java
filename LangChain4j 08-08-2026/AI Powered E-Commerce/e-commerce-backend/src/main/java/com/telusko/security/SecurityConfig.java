...
public class SecurityConfig {
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(auth -> auth
                ...
                // Prometheus scrapes without credentials, so this endpoint has to be open.
                // It exposes operational data (endpoint names, latencies, token usage), so in a
                // real deployment do NOT leave it reachable from the internet: move Actuator to
                // its own port with management.server.port and firewall it to the monitoring
                // network, or put the scrape behind basic auth.
                ...
                // Admin support: manage all tickets + AI assistant
                .requestMatchers(
                ...
                // ---------- AI ----------
                // The analytics assistant reports store-wide revenue and stock, so it is admin
                // only. It must be listed before the general /ai/** rule below.
                .requestMatchers("/api/v1/ecommerce/ai/admin/**").hasRole("ADMIN")

                // Customer-facing AI features: any signed-in shopper.
