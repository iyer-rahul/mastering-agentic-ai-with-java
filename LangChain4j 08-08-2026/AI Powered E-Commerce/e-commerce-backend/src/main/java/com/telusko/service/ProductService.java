...
public class ProductService {
    ...
    /**
     * Memory-free client. Description generation and product search are one-shot calls, and the
     * conversational client's memory advisor rejects them for having no conversation id - which is
     * why AI product search failed before this was split out.
     */
    ...
    // Public so the AI shopping features can return the same product shape the REST endpoints do,
    // instead of maintaining a second mapping that could drift.
    ...
    /**
     * How many products are offered to the model as candidates.
     * <p>
     * Retrieval only has to get the right products into this pool; the model does the choosing. Five
     * was too tight to be a search feature - it capped every result set at five items no matter how
    ...
    public List<ProductResponseDto> semanticSearchProducts(String userQuery) {
        ...
        try {
            ...
            Generation generation = aiMetrics.record("product-search", () -> chatClient.prompt(prompt)
                    .call()
                    .chatResponse()
                    .getResult());
